...
export function ProductMarquee({
  ...
  // Kesintisiz döngü için listeyi iki kez basıyoruz (-%50 kaydırma tam bir kopyaya denk gelir).
  const items = [...products, ...products]
  const style = { '--marquee-duration': `${durationSeconds}s` } as CSSProperties

  return (
    <div className="group relative overflow-hidden py-1">
      <div className="flex w-max animate-marquee gap-5 group-hover:[animation-play-state:paused]" style={style}>
        {items.map((product, i) => {
          ...
          return (
            <Link
              ...
              className="group/card relative w-44 shrink-0 overflow-hidden rounded-xl border border-line bg-paper shadow-soft transition sm:w-52"
            >
              <div className="relative aspect-square overflow-hidden bg-white">
                ...
                />
                {category && (
                  <span className="absolute left-2 top-2 rounded-full bg-ink/75 px-2.5 py-1 text-[0.65rem] font-medium text-cream opacity-0 backdrop-blur transition group-hover/card:opacity-100">
                    {category.name}
                  </span>
                )}
              </div>
              <div className="border-t border-line px-3 py-2.5">
                ...
                <p className="truncate font-serif text-sm font-semibold text-ink">{product.title}</p>
              </div>
            </Link>
          )
        })}
